import { Head, Link, useForm } from '@inertiajs/react';
import AdminLayout from '@/layouts/admin/admin-layout';
import { Button } from '@/components/ui/button';
import { Dialog, DialogTrigger, DialogContent, DialogHeader, DialogTitle, DialogDescription, DialogFooter, DialogClose } from '@/components/ui/dialog';
import { Input } from '@/components/ui/input';
import { Label } from '@/components/ui/label';
import { useState } from 'react';

type Patient = { id: number | string; name: string; uhid?: string; phone?: string };

type Paginated<T> = { data: T[]; links?: Array<{ url: string | null; label: string; active: boolean }>; meta?: any };

type Props = { patients: Paginated<Patient>; filters?: { q?: string; lab_id?: string } };

export default function PatientsIndex({ patients, filters = {} }: Props) {
    const [open, setOpen] = useState(false);
    const [editing, setEditing] = useState<any | null>(null);

    const form = useForm({ name: '', uhid: '', phone: '', lab_id: '' });

    function openCreate() {
        setEditing(null);
        form.reset();
        setOpen(true);
    }

    function openEdit(p: any) {
        setEditing(p);
        form.setData({ name: p.name || '', uhid: p.uhid || '', phone: p.phone || '', lab_id: p.lab_id || '' });
        setOpen(true);
    }

    function submit(e: any) {
        e.preventDefault();
        if (editing) {
            form.put(`/admin/patients/${editing.id}`, { onSuccess: () => setOpen(false) });
        } else {
            form.post('/admin/patients', { onSuccess: () => setOpen(false) });
        }
    }

    return (
        <AdminLayout breadcrumbs={[{ title: 'Patients' }] }>
            <Head title="Patients" />

            <div className="mb-6 flex items-center justify-between p-4 rounded-md bg-white text-black">
                <div>
                    <h1 className="text-2xl font-semibold">Patients</h1>
                    <div className="text-sm text-muted-foreground">Manage patients</div>
                </div>

                <div className="flex items-center space-x-3">
                    <form method="get" action="/admin/patients" className="flex items-center space-x-2">
                        <input name="q" defaultValue={filters.q ?? ''} placeholder="Search patients" className="rounded-md border px-3 py-2" />
                        <Button type="submit" variant="secondary">Filter</Button>
                    </form>

                    <Dialog open={open} onOpenChange={setOpen}>
                        <DialogTrigger asChild>
                            <Button onClick={openCreate}>Create patient</Button>
                        </DialogTrigger>

                        <DialogContent>
                            <DialogHeader>
                                <DialogTitle>{editing ? 'Edit patient' : 'Create patient'}</DialogTitle>
                                <DialogDescription>{editing ? 'Update patient details' : 'Create a new patient'}</DialogDescription>
                            </DialogHeader>

                            <form onSubmit={submit} className="grid gap-4">
                                <div>
                                    <Label htmlFor="name">Name</Label>
                                    <Input id="name" name="name" value={form.data.name} onChange={(e: any) => form.setData('name', e.target.value)} required />
                                </div>

                                <div>
                                    <Label htmlFor="uhid">UHID</Label>
                                    <Input id="uhid" name="uhid" value={form.data.uhid} onChange={(e: any) => form.setData('uhid', e.target.value)} />
                                </div>

                                <div>
                                    <Label htmlFor="phone">Phone</Label>
                                    <Input id="phone" name="phone" value={form.data.phone} onChange={(e: any) => form.setData('phone', e.target.value)} />
                                </div>

                                <DialogFooter>
                                    <Button type="submit">{editing ? 'Save' : 'Create'}</Button>
                                    <DialogClose asChild>
                                        <Button variant="ghost">Cancel</Button>
                                    </DialogClose>
                                </DialogFooter>
                            </form>
                        </DialogContent>
                    </Dialog>
                </div>
            </div>

            <div className="overflow-hidden rounded-md border bg-white text-black">
                <table className="w-full table-auto text-left">
                    <thead>
                        <tr>
                            <th className="px-4 py-2">Name</th>
                            <th className="px-4 py-2">UHID</th>
                            <th className="px-4 py-2">Phone</th>
                            <th className="px-4 py-2">Actions</th>
                        </tr>
                    </thead>
                    <tbody>
                        {patients.data.map((p) => (
                            <tr key={p.id} className="border-t">
                                <td className="px-4 py-3">{p.name}</td>
                                <td className="px-4 py-3">{p.uhid}</td>
                                <td className="px-4 py-3">{p.phone}</td>
                                <td className="px-4 py-3 space-x-2">
                                    <Button variant="secondary" onClick={() => openEdit(p)}>Edit</Button>
                                    <form action={`/admin/patients/${p.id}`} method="post" className="inline-block">
                                        <input type="hidden" name="_method" value="delete" />
                                        <Button type="submit" variant="destructive">Delete</Button>
                                    </form>
                                </td>
                            </tr>
                        ))}
                    </tbody>
                </table>
            </div>

            {patients.links && (
                <nav className="mt-4">
                    <ul className="inline-flex items-center space-x-2">
                        {patients.links.map((link, idx) => (
                            <li key={idx}>
                                {link.url ? (
                                    <Link href={link.url} className={link.active ? 'font-bold' : ''} dangerouslySetInnerHTML={{ __html: link.label }} />
                                ) : (
                                    <span dangerouslySetInnerHTML={{ __html: link.label }} className="text-muted-foreground" />
                                )}
                            </li>
                        ))}
                    </ul>
                </nav>
            )}
        </AdminLayout>
    );
}
